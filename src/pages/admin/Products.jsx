...
const AdminProducts = () => {
  ...
  const handleOpenModal = (product = null) => {
    if (product) {
      setFormData({ name: product.name, description: product.description || '', categoryId: product.categoryId?._id || '', totalStock: product.totalStock });
      setSelectedProduct(product);
    } else {
      setFormData({ name: '', description: '', categoryId: categories[0]?._id || '', totalStock: 0 });
    ...
    <div className="flex flex-col gap-6">
      ...
      <Modal isOpen={isStockModalOpen} onClose={() => setIsStockModalOpen(false)} title="Adjust Stock Quantity">
        <form onSubmit={handleStockSubmit} className="flex flex-col gap-4">
          <p className="text-sm text-text-muted">Adjusting stock for: <strong className="text-text">{selectedProduct?.name}</strong></p>
          <div className="bg-surface-hover/50 p-4 rounded-md border border-border flex justify-between">
            <span>Current Total: <strong>{selectedProduct?.totalStock}</strong></span>
            <span>Current Available: <strong>{selectedProduct?.availableStock}</strong></span>
          </div>
          <FormInput label="Adjustment Amount (e.g. +50 or -10)" id="stock" type="number" placeholder="0" value={stockAdjustment} onChange={e => setStockAdjustment(e.target.value)} required />
